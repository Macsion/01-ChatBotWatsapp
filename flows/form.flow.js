...
const formFlow = addKeyword(EVENTS.ACTION)
    ...
    .addAnswer("¡Gracias por la información! Seras contactado a la brevedad \nEscribe *Menu* para volver a ver las opciones", null,
        async (ctx, ctxFn) => {
            const userInfo = await ctxFn.state.getMyState(); // Recupera todos los datos almacenados en el estado
            await appendToSheet([[userInfo.name, userInfo.mail, userInfo.motive, ctx.from]])
        });
